...
cxb=[0 1]; %breakpoint for cx fit spline 
...
teb=[0 1]; %breakpoint for te fit spline 
ten=6; %spline polynomial order for the te fit spline
...
doit=1 %input('Enter 1 to input fits from previous JETnewfit run into CRONOS data file: '); %if satisfied with the fit, then doit
if doit==1
    data.prof.ti=titemp;
...
runnum=77914 %input('Which run suffix? ');
cxtime=[45.7, 50.44] %input('When does the beam turn [on off]? ');
